...
def strTeam(teamNum):
    return region+'-'+age+gender+'-'+str(firstTeamNum+teamNum)
...
with open(outputFileName, 'w') as outFile:
    for week in weekRange:
        ...
        for field in range(0,len(fieldTimes)):
            ...
            outFile.write('\t' +strTeam(thisField[0]) + '\t'+ strTeam(thisField[1]))
            outFile.write('\n')
...
with open(outputWeekByWeekFileName, 'w') as outWeekFile:
    ...
    for field in range(0,len(fieldTimes)):
        ...
        for week in weekRange:
            ...
            ww = week % (numTeams - 1)
            thisField=weeklyMap[ww][field]
            outWeekFile.write(',' +strTeam(thisField[0]) + ','+ strTeam(thisField[1]))
        outWeekFile.write('\n')

    outWeekFile.write('\n\n\n')
    outWeekFile.write('Team,#Home,#Away,#Total\n')
    for teamNum in range(0, numTeams):
        outWeekFile.write(strTeam(teamNum))
